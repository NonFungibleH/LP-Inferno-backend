...
export async function scanVault() {
  ...
  for (const manager of MANAGERS) {
    ...
    const iface = new ethers.Interface(pmAbi);
    const tokenIdsInVault = new Set<string>();

    for (const log of logs) {
      try {
        const parsed = iface.parseLog(log);
        const tokenId = parsed.args.tokenId.toString();
        tokenIdsInVault.add(tokenId);
      } catch {
        continue;
      }
    }

    for (const tokenId of tokenIdsInVault) {
      try {
        const owner = await pm.ownerOf(tokenId);
        if (owner.toLowerCase() !== VAULT.toLowerCase()) continue;

        const sender = await inferno.originalOwner(manager.address, tokenId);
        data.push({
          tokenId,

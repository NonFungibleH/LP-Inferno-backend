...
async function fetchPositionTokens(
  ...
  if (isV4) {
    ...
    // Check for MigrationInitiated event
    try {
      const searchStartBlock = Math.max(START_BLOCK, burnBlock - FALLBACK_CHUNK / 2);
      const searchEndBlock = burnBlock + FALLBACK_CHUNK / 2;
      console.log(`🔍 Checking MigrationInitiated events for tokenId ${tokenId} from block ${searchStartBlock} to ${searchEndBlock}`);
      const logs = await safeGetLogs(
        provider,
        {
          address: VAULT_ADDRESS,
          fromBlock: searchStartBlock,
          toBlock:=YamlArtifact{ searchEndBlock,
          topics: [
            ethers.id("MigrationInitiated(address,address,uint256,uint256)"),
            null, // user
      ...
      if (logs.length > 0) {
        for (const log of logs) {
          const [, , , , newTokenId] = ethers.AbiCoder.defaultAbiCoder().decode(
            ["address", "address", "uint256", "uint256"],
            log.data
          );
          console.log(`ℹ️ Found MigrationInitiated event for tokenId ${tokenId} to newTokenId ${newTokenId} at block ${log.blockNumber}, tx: ${log.transactionHash}`);
          // Try positions with newTokenId
          try {
            const abi = [
              "function positions(uint256) view returns (uint96,address,address,address,uint24,int24,int24,uint128,uint256,uint256,uint128,uint128)"
            ];
            const contract = new ethers.Contract(manager, abi, provider);
            const pos = await contract.positions(newTokenId);
            console.log(`✅ Fetched position data for newTokenId ${newTokenId} on manager ${manager}`);
            return { token0: pos[2], token1: pos[3] };
          } catch (e) {
            console.warn(`⚠️ positions call failed for newTokenId ${newTokenId} on manager ${manager}:`, e);
          }
        }
      }
      console.warn(`⚠️ No MigrationInitiated event found for tokenId ${tokenId} from block ${searchStartBlock} to ${searchEndBlock}`);
    } catch (e) {
      console.warn(`⚠️ MigrationInitiated fetch error for tokenId ${tokenId}:`, e);
    }

    // Fallback to tracing NFTBurned transaction for mint event
    try {
        ...
        for (const log of receipt.logs) {
          if (log.address.toLowerCase() === manager.toLowerCase()) {
            try {
              ...
              } else if (parsed && parsed.name === "IncreaseLiquidity" && parsed.args.tokenId.toString() === tokenId) {
                console.log(`ℹ️ Found IncreaseLiquidity event for tokenId ${tokenId} in tx ${txHash}`);
                // Fetch pool address from mint transaction to get token0, token1
                const mintTxHash = receipt.transactionHash; // Use the mint tx identified earlier
                const mintReceipt = await provider.getTransactionReceipt(mintTxHash);
                if (mintReceipt && mintReceipt.logs) {
                  for (const mintLog of mintReceipt.logs) {
                    try {
                      ...
                      const poolParsed = poolIface.parseLog(mintLog);
                      if (poolParsed && poolParsed.name === "PoolCreated") {
                        console.log(`✅ Found PoolCreated event in mint tx ${mintTxHash}: token0=${poolParsed.args.token0}, token1=${poolParsed.args.token1}`);
                        return { token0: poolParsed.args.token0, token1: poolParsed.args.token1 };
                      }
                    } catch (e) {
